...
cache = get_cache()
# Email sending is optional for tests. Import fastapi_mail lazily inside send_email_async.
conf = None
...
# Logging slow queries via SQLAlchemy event (best-effort)
...
async def auth(data: AuthRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    if data.register:
        ...
        try:
            ...
            # send_email_task returns AsyncResult when celery/broker configured or None
